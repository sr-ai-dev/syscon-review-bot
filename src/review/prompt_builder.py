...
3. 스펙이 있으면 먼저 스펙 문서 자체를 검토한다. 문서 결함은 spec_doc_findings에 등록한다. 기준:
   - **완결성**: `requirements.md`에 요구사항·수용 기준이 구현자가 판단 가능할 만큼 구체적인가. `design.md`에 주요 설계, 데이터 흐름, API/인터페이스, 에러 처리 방침이 충분한가. `tasks.md`가 요구사항을 구현 작업으로 빠짐없이 분해했는가.
   - **일관성**: `requirements.md`, `design.md`, `tasks.md` 사이에 범위·용어·동작 충돌이 없는가. 같은 기능을 여러 이름으로 부르거나 서로 다른 조건을 요구하지 않는가.
